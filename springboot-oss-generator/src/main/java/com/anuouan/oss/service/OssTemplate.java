...
public class OssTemplate implements InitializingBean, FileTemplate {
	 ...
	 */
	@SneakyThrows
	public String getTemporaryObjectURL(String bucketName, String objectName, Integer expires) {
		Date date = new Date();
	 ...
	 */
	@SneakyThrows
	public String getPermanentObjectURL(String bucketName, String objectName, Integer expires) {
		Date date = new Date();
		Calendar calendar = new GregorianCalendar();
		calendar.setTime(date);
		calendar.add(Calendar.DAY_OF_MONTH, expires);
	...
	public void afterPropertiesSet() {
		// 创建一个ClientConfiguration对象，用于配置S3客户端的参数，如最大连接数等
		ClientConfiguration clientConfiguration = new ClientConfiguration();
		// 从配置文件中获取最大连接数，并设置到ClientConfiguration对象中
		clientConfiguration.setMaxConnections(properties.getOss().getMaxConnections());
		// 创建一个EndpointConfiguration对象，用于配置S3服务的端点和区域。端点和区域信息从配置文件中获取
